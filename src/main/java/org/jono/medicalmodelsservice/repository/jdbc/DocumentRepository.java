...
public class DocumentRepository {
    ...
    public Document updateById(String id, DocumentDto documentDto, Map<SqlIdentifier, Object> updateMap) {
        // Fetch the existing document by ID
        Document existingDocument = documentCrudRepository.findById(id)
    ...
    public Tuple2<List<DocumentChild>, List<Document>> getDocsAndDocChildren() {
        List<DocumentChild> documentChildren = ImmutableList.copyOf(documentChildCrudRepository.findAll());
        List<Document> documents = ImmutableList.copyOf(documentCrudRepository.findAll());
