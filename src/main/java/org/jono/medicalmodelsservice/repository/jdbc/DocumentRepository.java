...
public class DocumentRepository {
    ...
    public List<ModelRanking> getModelTypeFrequencyForCompany(final String companyId) {
        final String sql = """
                       WITH RECURSIVE numbers AS (
                           SELECT 0 AS n
                           UNION ALL
                       ...
                       json_types AS (
                           SELECT JSON_UNQUOTE(JSON_EXTRACT(d.body, CONCAT('$[', n.n, '].type'))) as type_value
                           FROM document d
                           INNER JOIN document_company_relationship dcr ON d.id = dcr.document_id
                           CROSS JOIN numbers n
                           WHERE JSON_EXTRACT(d.body, CONCAT('$[', n.n, '].type')) IS NOT NULL
                           AND dcr.company_id = :companyId
                       )
                       SELECT 
                           type_value as type,
                           COUNT(*) as frequency
    ...
    public List<UserIdRanking> getCreatorFrequencyForDocument(final String companyId) {
        final String sql = """
                       SELECT 
                           d.creator as user_id,
                           COUNT(d.id) as document_count
                       FROM document d
                       INNER JOIN document_company_relationship dcr ON d.id = dcr.document_id
                       WHERE dcr.company_id = :companyId
                       GROUP BY d.creator
                       ORDER BY document_count DESC, user_id;
                       """;
    ...
    public long countByCompany(final String companyId) {
        return documentCrudRepository.countByCompanyId(companyId);
    }

    public List<DailyResourceCount> getDocumentGrowthDataByCompany(final String companyId) {
        final String sql = """ 
                       WITH RECURSIVE dates AS (
                           SELECT CAST(MIN(d.created_date) AS DATE) as date
                           FROM document d
