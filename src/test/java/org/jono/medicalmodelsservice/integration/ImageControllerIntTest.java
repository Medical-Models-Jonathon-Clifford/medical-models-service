...
class ImageControllerIntTest {
    ...
    void shouldUploadAndDownloadImage() throws Exception {
        ...
        // Upload the image
        final String fileNameResponse = this.mockMvc.perform(multipart("/images")
                                     .file(imageFile)
        ...
        // Download the image
        final byte[] downloadedImage = this.mockMvc.perform(get("/images/{fileName}", fileNameResponse))
                .andExpect(status().isOk())
        ...
        // Downloads the same image the test uploaded.
        assertThat(downloadedImage).isEqualTo(imageContent);
    }
}
